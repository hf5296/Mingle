...
The coursework is designed to help you apply the concepts, software development methods, and
frameworks covered in class to the development of a Cloud Software as a Service (SaaS) application.
...
messages on a particular topic while others browse posts and perform fundamental interactions, such as
liking, disliking, or adding a comment. You should install, test, and document your developments as
described in Section 3.
Mingle should support the following actions.
Action 1: Authorised users access the Mingle API using the OAuth v2 protocol to perform any
interaction.
Action 2: Action 3: Action 4: Action 5: Authorised users post a message for a particular topic in the Mingle API.
Registered users browse messages per topic using the Mingle API.
Registered users perform basic operations, including “like”, “dislike”, or “comment” on
a message posted for a topic.
Authorised users could browse for the most active post per topic with the highest likes
and dislikes.
Action 6: Authorised users could browse the history data of expired posts per topic.
You are advised to use the Node.js Express package and MongoDB to develop your software. If you
like, you can use any other programming framework or database you prefer. You should use code
samples from the lab tutorials and online sources by providing the appropriate references (in text and
code). In the technical report, you can enhance it by describing your functionality and implementation
decisions, and by including a diagram to demonstrate your implementation.
1 / 7
2025-2026 BUCI028H6
3. Coursework phases
...
¨ The JWT will authorise the auctioning RESTful API to store data for authorised users
in your database.
¨ You can use a MongoDB database to store the data required for the coursework, as
demonstrated in the labs.
• Your code should authenticate users each time you perform any action in Part C, for example,
whenever users post, interact, or browse messages.
¨ Unauthorised users are not allowed to access your resources and perform database
requests.
...
¨ The post identifier.
¨ The title of a post.
¨ The topic of a post from one of the following four categories: Politics, Health, Sport or
Tech. Each post could belong to one or more topics.
¨ A timestamp of the post-registration in the Mingle API.
¨ The message body of a post.
2 / 7
2025-2026 BUCI028H6
¨ The post-expiration time. After this period, the message will remain on the Mingle wall,
but it will not accept any further actions, such as likes, dislikes, or comments.
...
• Each user interaction with a post on a topic (like, dislike or comment) should include the
following data:
...
• You are encouraged to develop your database collections and application logic further, and to
expand Phase C as needed.
¨ Provide a brief description of your database models in the report.
¨ Use screenshots if necessary.
• You should list the RESTful API endpoints in your report with a simple example of using your
API.
...
in the report or by developing a string application. For example, you can create a testing application
...
functionality.
Let us assume a use case scenario of four users, Olga, Nick, Mary, and Nestor, accessing the Mingle
API. Provide the following test cases (TCs).
3 / 7
2025-2026 BUCI028H6
TC 1. TC 2. TC 3. TC 4. TC 5. TC 6. TC 7. TC 8. TC 9. TC 10. TC 11. TC 12. Olga, Nick, Mary, and Nestor register and are ready to access the Mingle API.
Olga, Nick, Mary, and Nestor use the OAuth v2 authorisation service to register and get
their tokens.
Olga makes a call to the API without using her token. This call should be unsuccessful as
the user is unauthorised.
Olga posts a message in the Tech topic with an expiration time (e.g. 5 minutes) using her
token. After the expiration time, the message will not accept any further user interactions
...
Nick and Olga browse all the available posts in the Tech topic; three posts should be
...
Nick browses all the available posts on the Tech topic; at this stage, he can see the number
...
Mary likes her post on the Tech topic. This call should be unsuccessful; in Mingle, a post
owner cannot like their own messages.
Nick and Olga comment on Mary’s post on the Tech topic in a round-robin fashion (one
...
of likes and dislikes on each post and the comments made.
TC 14. TC 15. Nestor posts a message in the Health topic with an expiration time using her token.
Mary browses all the available posts on the Health topic; at this stage, she can see only
Nestor’s post.
TC 16. TC 17. TC 18. Mary posts a comment in Nestor’s message on the Health topic.
Mary dislikes Nestor’s message on the Health topic after the post expires. This should fail.
Nestor browses all the messages on the Health topic. There should be only one post (his
own) with one comment (Mary’s).
TC 19. TC 20. Nick browses all the expired messages on the Sports topic. These should be empty.
Nestor queries for an active post with the highest interest (maximum number of likes and
...
There is no specific report size, but you should keep the report concise and focused on the
development phases.
Phase G: Submit quality scripts [10 marks]
Your code should be written in such a way that makes it highly readable and easy to follow. Good use
of REST concepts, understanding of computational complexity in software development, use of
...
the Mingle functionalities. Using CI/CD pipelines with Terraform to deploy or develop a front-end could
...
• Follow the instructions in the coursework brief and description on the Moodle page regarding
the time and mode of submission for your software.
...
5 / 7
2025-2026 BUCI028H6
• The coursework requires you to create database tables in MongoDB to save your data, allowing
you to improvise as needed.
...
b) Deploy your code in the virtual machine using your
GitHub repository.
c) REST API endpoints should be available under your
virtual machine IP address based on examples and guidelines
seen in class. Provide screenshots.
